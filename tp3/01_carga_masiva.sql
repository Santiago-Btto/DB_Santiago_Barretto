...
-- Ejemplo: psql -X -v ON_ERROR_STOP=1 -v tp3_lote=SB2026TP3 -d food_store_tp3_lab -f tp3/01_carga_masiva.sql
\if :{?tp3_lote}
\else
...
SELECT format('TP3_%s_PRODUCTO_%s', :'tp3_lote', lpad(serie.n::text, 6, '0')),
       format('Producto generado para TP3, lote %s, secuencia %s', :'tp3_lote', serie.n),
...
FROM generate_series(1, 50000) AS serie(n)
JOIN producto
  ON producto.nombre = format('TP3_%s_PRODUCTO_%s', :'tp3_lote', lpad(serie.n::text, 6, '0'));
...
SELECT format('Cliente%s', lpad(serie.n::text, 5, '0')),
       ...
       format('tp3_%s_cliente_%s@example.invalid', :'tp3_lote', lpad(serie.n::text, 5, '0')),
...
FROM generate_series(1, 20000) AS serie(n)
JOIN cliente
  ON cliente.email = format('tp3_%s_cliente_%s@example.invalid', :'tp3_lote', lpad(serie.n::text, 5, '0'));
...
FROM cliente WHERE left(email, length(format('tp3_%s_cliente_', :'tp3_lote))) = format('tp3_%s_cliente_', :'tp3_lote')
UNION ALL
SELECT 'productos_lote', count(*)
FROM producto WHERE left(nombre, length(format('TP3_%s_PRODUCTO_', :'tp3_lote))) = format('TP3_%s_PRODUCTO_', :'tp3_lote')
UNION ALL
SELECT 'pedidos_lote', count(*)
FROM pedido ped JOIN cliente cli ON cli.id_cliente = ped.id_cliente
WHERE left(cli.email, length(format('tp3_%s_cliente_', :'tp3_lote))) = format('tp3_%s_cliente_', :'tp3_lote')
UNION ALL
